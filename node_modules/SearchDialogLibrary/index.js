...
function create(libraryId, settings) {
    ...
    library.dialog('/',
        new builder.SimpleDialog((session, args) => {
            ...
            if (hasInput) {
                ...
                // G. Prompt
                //searchPrompt(session);
                //searchPrompt2(session);
                // F. Perform search
                if (session.userData.question != null) {
                var newQuery = Object.assign({}, query, { searchText: session.userData.question });
                //    var newQuery = "billing analyzer 2.1";
                    performSearch(session, newQuery, selection);
                } else {
                    searchPrompt2(session);
                }
    ...
    library.dialog('/results',
        new builder.IntentDialog()
            .onBegin((session, args) => {
                ...
                console.log('The query', session.dialogData.query);
                console.log('The search Reponse', session.dialogData.searchResponse);
                console.log('The selection', session.dialogData.selection);
    ...
    // , new builder.CardAction().type('imBack').title('More info').value(searchHit.key) 
    ...
    function searchPrompt(session) {
        var prompt = 'What is your question for the ServiceNow Bot?';
    ...
    function searchPrompt2(session) {

        if (session.userData.product == "EY Delivers") {
            var prompt = 'I can answer FAQs related to EY Delivers.  Please ask me a question';
        } else {
            var prompt = 'I can answer FAQs.  Please ask me a question';
        }
        if (!!session.dialogData.firstTimeDone) {
            prompt = 'What is your next question?';
            if (settings.multipleSelection) {
